...
import TaskList from '../tasks/TaskList'; // Import the TaskList component
...
const ProjectDetails = () => {
    ...
    const fetchProjectDetails = async () => {
      try {
        ...
        if (projectData.manager_id) {
          const name = await employeeService.getEmployeeNameById(projectData.manager_id);
        ...
        try {
          const membersData = await projectService.getProjectMembers(projectId, token);
        ...
        try {
          const progressPercentage = await projectService.calculateProjectProgress(projectId, token);
  ...
  const handleRemoveMember = async (userId) => {
    try {
      ...
      // Update members list
      const updatedMembers = await projectService.getProjectMembers(projectId, token);
  ...
  const handleDeleteProject = async () => {
    try {
      ...
      navigate('/projects', { state: { message: 'Project successfully deleted' } });
    } catch (err) {
  ...
  const handleAddMember = async () => {
    if (emailInputMode) {
      ...
      try {
        const emails = memberEmails.split(',').map(email => email.trim()).filter(email => email);
        
        // Process each email
        const addPromises = emails.map(async (email) => {
          try {
            // Find user by email
            const user = await employeeService.findEmployeeByEmail(email, token);
            if (!user) {
  ...
  // Loading state
  if (loading) {
    return <div className="flex justify-center items-center h-[50vh]">Loading project details...</div>;
  }
    ...
    <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <span className={`inline-block px-2.5 py-0.5 rounded text-xs font-medium ${getPriorityClass(project.priority)}`}>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="flex mb-4">
              {members.slice(0, 5).map((member, index) => (
                <div 
                  key={member.user_id} 
                  className={`w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-gray-600 font-medium border-2 border-white ${index > 0 ? '-ml-2' : ''}`}
      ...
      {showAddMemberDialog && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowAddMemberDialog(false)}></div>
            ...
            <div className="bg-white rounded-lg shadow-lg max-w-md w-full relative z-10" onClick={e => e.stopPropagation()}>
              <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-gray-800">Add Project Members</h2>
                  <button 
                  ...
                  {emailInputMode ? (
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mb-4">
                        Add multiple members by entering their email addresses separated by commas
                      </p>
                    </div>
                  ) : (
                    <div>
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button 
                    ...
                    disabled={emailInputMode ? !memberEmails.trim() : !memberToAdd}
                  >
